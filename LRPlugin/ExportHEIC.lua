...
return {
	...
	sectionForFilterInDialog = function(viewFactory, propertyTable)
		...
		if propertyTable.HEICColorSpace == "DisplayP3_PQ" then
			propertyTable.bitDepthEnabled = false
		else
			propertyTable.bitDepthEnabled = true
		end
				...
				f:column({ -- left-column
					...
					f:row({ -- control 2: color space
						...
						f:popup_menu({
							...
							items = {
								...
								{ separator = true },
								{ title = "Display P3 (HDR)", value = "DisplayP3_PQ" },
							},
					...
					f:row({ -- control 3: bit depth
						...
						f:radio_button({
							...
							title = "8",
							checked_value = 8,
							enabled = LrBinding.keyIsNot("HEICColorSpace", "DisplayP3_PQ"),
						}),
						f:radio_button({
							value = bind("HEICBitDepth"),
							title = "10",
	...
	postProcessRenderedPhotos = function(functionContext, filterContext)
		...
		local renditionOptions = {
			filterSettings = function(renditionToSatisfy, exportSettings)
				...
				elseif p.HEICColorSpace == "DisplayP3_PQ" then
					-- Undocumented HDR flags
		...
		if p.HEICColorSpace == "DisplayP3_PQ" then
			cmd = cmd .. " --color-space DisplayP3_PQ"
		end
		...
		for sourceRendition, renditionToSatisfy in filterContext:renditions(renditionOptions) do
			...
			if success then
				...
				logger:info("Running command: " .. actualCmd)
				...
				-- LrTasks.execute("rm " .. pathOrMessage)
			else
