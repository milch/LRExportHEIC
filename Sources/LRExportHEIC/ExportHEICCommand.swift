...
struct ExportHEICCommand: Command {
    public struct ExportHEICCommandSignature: CommandSignature {
        ...
        @Option(name: "quality", help: "Compression quality between 0.0-1.0", required: true, allowedValues:0.0...1.0)
        var quality: Float!

        @Option(name: "color-space", help: "Name of the output color space. Omit to use input image color space", allowedValues: [
            CGColorSpace.sRGB,
    ...
    func run(using context: CommandContext, signature: ExportHEICCommandSignature) throws {
        ...
        let colorSpace = signature.colorSpace ?? inputImage.colorSpace ?? CGColorSpace(name: CGColorSpace.sRGB)!
        ...
        let ctx = CIContext()
        let opts = [kCGImageDestinationLossyCompressionQuality: signature.quality] as [CIImageRepresentationOption: Any]
